...
namespace CombatSystem
{
    public class EnemyDetect : MonoBehaviour
    {
        private void OnTriggerEnter2D(Collider2D collision)
        {
            ...
            if (collision.TryGetComponent(out EntityCharacter entityCharacter)) entityCharacter.RegisterObject();
        }

        private void OnTriggerExit2D(Collider2D collision)
        {
            if (!IsColliderHaveEnemyTag(collision)) return;
            if (collision.TryGetComponent(out EntityCharacter entityCharacter))
            {
